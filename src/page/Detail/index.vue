            ...
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  changepirce="0"
                  @click="
                    selectActive(attr.spuSaleAttrValueList, attrVal, index)
                  "
                  :class="{ active: attrVal.isChecked == 1 }"
                  v-for="(attrVal, index) in attr.spuSaleAttrValueList"
                  :key="index"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  v-model="saleVal"
                  @change="handleSaleVal"
                />
                <a class="plus" @click="saleVal++">+</a>
                <a class="mins" @click="saleVal > 1 ? saleVal-- : 1">-</a>
              </div>
              <div class="add">
                <a @click="handleShopCar">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    // 当页面一挂载就发送请求
    this.$store.dispatch("getDetail", this.$route.params.sku);
  },
  computed: {
  ...
  methods: {
    // 处理选中事件，排他思想，所有置0，在对点击的置1
    selectActive(arr, curVal, index) {
      arr.forEach((item) => (item.isChecked = 0));
      curVal.isChecked = 1;
      if(arr[index].saleAttrName == '颜色')
      {
        this.color = arr[index].saleAttrValueName;
      }else{
        this.version = arr[index].saleAttrValueName;
      }
    },
    // 处理商品减少事件
    handleSaleVal(event) {
      ...
      if (isNaN(value) || value < 1) {
        ...
        // 需要对value进行取整，如果输入的数字为非整数
        this.saleVal = parseInt(value);
      }
    },
    // 处理购物车事件
    async handleShopCar() {
      /*
            1.发送请求给服务器，将产品的信息添加到服务器上
            2.如果添加成功则进行路由跳转
            3.如果返回失败则进行提示用户出错了 
         */
      // 第一个参数商品的id，第二个参数是商品的数量
      /* 
          下面这串代码实际上就是调用actions下的addShopCar方法，而
          该方法被async和await包裹，返回的一定是promise
          try...catch进行捕获，如果成功进行路由跳转，如果失败则提示用户
        */
      try {
        ...
        // 进行路由跳转
        /*
              push传参，由于skuInfo传的数据复杂所以地址栏会有很长的乱码，虽然可以用
              但是为了不那么恶心，可以采用会话存储的方式
              以后简单的数据可以使用query的方式给路由组件
              复杂的数据可以使用会话存储，这里为什么不是本地存储，因为该数据只展示一次所以没有必要
           */
        this.$router.push({
          name: "shop-addcartsuccess",
          query: { saleVal: this.saleVal, color: this.color,version:this.version },
        });
        // 需要转JSON才能进行存储
        sessionStorage.setItem("ADDCARTINFO", JSON.stringify(this.skuInfo));
      } catch (error) {
        alert(error.message);
      }
    },
